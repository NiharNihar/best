...
GraphNode *cloneGraph2(GraphNode *src)
{
	map<GraphNode*, GraphNode*> gm;
	queue<GraphNode*> gq;
	...
	gq.push(src);
	while(!gq.empty()) {
		GraphNode* cur = gq.front();
		gq.pop();
		for ( auto cn : cur->neighbours) {
			if (gm[cn] == NULL) {
				GraphNode *nn;
				nn = new GraphNode();
				nn->val = cn->val;
			...
			gm[cur]->neighbours.push_back(gm[cn]);
		}
...
{ 
	...
	bfs(newsrc); 
	GraphNode *newsrc2 = cloneGraph2(src); 
	cout << "BFS Traversal after cloning\n"; 
	bfs(newsrc2); 
	return 0; 
} 
